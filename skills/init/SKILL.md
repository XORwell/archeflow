...
5. Say that merges into their branch are confirmed interactively by default (`git.auto_merge: false`), and that `.archeflow/` holds run state they normally do not commit (the script writes `.archeflow/.gitignore`).
