...
| manual only | `commit <run_id> <phase> "<msg>" [files]`, `phase-commit <run_id> <phase>`, `rollback <run_id> --to <phase> [--yes]` | low-level tools a run does not use: commit files on the run branch yourself, and reset the run branch to such a commit |

The merge commit subject is `archeflow: merge run <run_id>` (a neutral type: the run may be a fix,
a feature or a refactor). `archeflow-rollback.sh` reverts only a HEAD commit with exactly that
subject, or the subject older versions wrote, `feat: archeflow run <run_id> complete`.
...
fallback. `enabled` and `auto_merge` are read by the agent following `archeflow:run`; the
others by the script.
...
are run state, not source. A run commits nothing under `.archeflow/`, so a merge brings only the
Maker's changes into the base branch, and the `.archeflow/.gitignore` written by
`/archeflow:init` keeps run state out of your own commits. The audit trail is the event log
(`.archeflow/events/<run_id>.jsonl`), not git history.
