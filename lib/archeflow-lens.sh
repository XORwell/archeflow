...
yaml_to_json() {
  ...
  if command -v yq >/dev/null 2>&1 && yq --version 2>&1 | grep -qi mikefarah; then
    ...
    # File path passed via argv, never interpolated into the Python source.
    python3 -c 'import sys, json, yaml; print(json.dumps(yaml.safe_load(open(sys.argv[1]))))' "$file"
  else
    "$ARCHEFLOW_ROOT/lib/archeflow-yaml.sh" "$file"
  fi
}
...
# "~/.ssh/id_ed25519" or "../../.aws/credentials" would leak local secrets into
# prompts (and from there to model providers / logs). Every entry must be a
# relative path without "..", and if it exists it must resolve inside the
# project root (no symlink escape). Prints the offending entry and returns 1.
validate_context_inject() {
  local merged="$1" root entry resolved
  ...
  while IFS= read -r -d '' entry; do
    if [[ -z "$entry" || "$entry" == /* || "$entry" == "~"* || "$entry" == -* \
          ...
          || "$entry" =~ [[:cntrl:]] ]]; then
      echo "error: unsafe context_inject path '$entry' (must be relative to the project, without '..')" >&2
      return 1
    fi
...
cmd_merge() {
  ...
  for name in "$@"; do
    ...
    # dependency-free converter (lib/archeflow-yaml.sh). Lens merging must work
    # on a stock runner with only bash/awk/jq.
    local json
    json="$(yaml_to_json "$file")" || { echo "error: could not parse lens $file" >&2; exit 1; }
